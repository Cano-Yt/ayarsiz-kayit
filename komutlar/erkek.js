...
let isim = args[1]
let yaş = args[2]
if(!message.member.roles.cache.has(ayarlar.kayıtsız)) return message.channel.send(`Bu komutu kullanabilmek için \`${message.guild.roles.cache.get(ayarlar.kayıtsız).name}\` Rolüne sahip olmalısın`)
if(!member) return message.channel.send(`Bir kullanıcı etiketle veya idsini gir`)
if(!member.roles.cache.has(ayarlar.kayıtyetkili)) return message.channel.send(`Kullanıcıda \`${message.guild.roles.cache.get(ayarlar.kayıtyetkili).name}\` Rolü olması lazım`)
